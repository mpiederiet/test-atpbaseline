...
$TestDefinition=[ATPBaselineCheck]@{
    'Name'='High Confidence Phish Action'
    'Control'=$MyFileName.BaseName
    'TestDefinitionFile'=$MyFileName.FullName
    'Area'='Content Filter Policies'
    'PassText'='High Confidence Phish action set to Quarantine message'
    'FailRecommendation'='Change High Confidence Phish action to Quarantine message.'
    'Importance'='It is recommended to configure the High Confidence Phish detection action to Quarantine so that these emails are not visible to the end user from within Outlook. As Phishing emails are designed to look legitimate, users may mistakenly think that a phishing email in Junk is false-positive.'
...
ForEach($Policy in $HostedContentFilterPolicy) {
    ...
    $Null=$Return.Add((New-ReturnObject -Inputobject $Policy -ObjectName 'Content Filter Policy' -NameProperty 'Name' -Property 'HighConfidencePhishAction' -TestScript {$InputObject.$Property -eq 'Quarantine'}))
}
