...
$TestDefinition=[ATPBaselineCheck]@{
    'Name'='Zero Hour Autopurge Enabled for Phish'
...
ForEach($Policy in $HostedContentFilterPolicy) {
    $Null=$Return.Add((New-ReturnObject -Inputobject $Policy -ObjectName 'Content Filter Policy' -NameProperty 'Name' -Property 'PhishZapEnabled' -TestScript {$InputObject.$Property -eq $True}))
}
