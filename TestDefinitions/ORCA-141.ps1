...
ForEach($Policy in $HostedContentFilterPolicy) {
    ...
    $Null=$Return.Add((New-ReturnObject -Inputobject $Policy -ObjectName 'Content Filter Policy' -NameProperty 'Name' -Property 'BulkSpamAction' -TestScript {$InputObject.$Property -eq 'MoveToJmf'}))
}
