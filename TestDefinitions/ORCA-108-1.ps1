...
ForEach($Domain in $AcceptedDomain) {
    ...
    If($Domain.Name -notlike '*.onmicrosoft.com') {
        ...
        If($null -ne $DkimSigning) {
            ...
            $Selector1=$Selector1|Where-Object{$null -ne $_ -and $_.Section -eq 'Answer' -and $_.Type -eq 'CNAME'}
            if($null -ne $Selector1 -and $Selector1.NameHost -eq $DkimSigning.Selector1CNAME) {
            ...
            $Selector2=$Selector2|Where-Object{$null -ne $_ -and $_.Section -eq 'Answer' -and $_.Type -eq 'CNAME'}
            if($null -ne $Selector2 -and $Selector2.NameHost -eq $DkimSigning.Selector2CNAME) {
                $Null=$Return.Add([PSCustomObject][Ordered]@{
                    'Domain'=$Domain.Name
